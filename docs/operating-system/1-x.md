...
> [虚拟地址空间](./3-1-x.md#virtual-address-space){target="_blank"}将会在后面的章节中详细讲解，读者可以考虑访问链接简单瞥一眼留个印象。

下图是进程在虚拟地址空间中用户部分的组成：

!!! section "用户部分"

    <center> ![Layout of a process in memory.](imgs/9.png){ align=right width=35% } </center>
    ...
    ??? eg "memory layout of a C program"
        ...
        ![Memory layout of a C program.](imgs/10.png)
...
!!! section "内核部分"
    **进程控制块(process control block, PCB)**是操作系统中用来描述进程的一种数据结构，它包含了进程的所有信息，是操作系统中最重要的数据结构之一。可以说，PCB 是进程存在的唯一标志。
    ...
    <center> ![Process control block (PCB).](imgs/11.png){ align=right width=35% } </center>
...
<center> ![Diagram of process state.](imgs/12.png) </center>
...
![The ready queue and wait queues.](imgs/17.png)

实际实现中，由于等待的 I/O 或事件不同，可能维护多个等待队列，于是实际过程中的情况可能如下：

![Queueing-diagram representation of process scheduling.](imgs/15.png)
...
![A tree of processes on a typical Linux system.](imgs/13.png)
...
!!! key-point "关于带 `fork` 的程序分析是一个比较重要的点，请确保自己能比较流畅地分析相关程序。"
...
如果第 18 行仍然被注释，那么 parent 进程和 child 进程将并**发**执行，即完成 `fork()` 后两个进程都从 11 行开始继续向下并发的执行，互不阻塞；如果 18 行的注释被取消，那么 parent 进程将等待 child 进程结束后再继续。读者可以尝试排列组合两个注释来观察程序运行的结果会如何变化。
...
!!! extra "copy on write"
    ...
    > 本块内容于[虚拟内存](./3-2-x.md#回顾-copy-on-write){target="_blank"}相关的内容有关，如果读者想不明白具体实现，可以先了解功能，在之后学习相关内容后我会再次提醒读者回顾该部分内容。

<a id="vfork">
!!! extra "virtual memory fork"
    ...
    !!! question "可是这有啥用呢？我创建一个 reference 做的事情和我直接做的事情没啥区别啊？"
...
当进程调用 `exit()` 这个系统调用时，这个进程将被终止，这意味着这个进程将不再执行，其资源将被释放，同时返回状态值，而这个状态值将被 parent 进程的 `wait()` 接收。特别的，如果 parent 进程尚未调用 `wait()`，则这个 child 进程还不会完全消失，因为要返回的东西还没返回（child 并不知道 parent 会不会、什么时候来回收它）。这种逻辑上已经终止，但仍然占有一部分资源，等待 parent 进程调用 `wait()` 的进程，我们称之为**僵尸进程(zombie)**。一个进程变成僵尸进程是很常见的。关键在于，如果 parent 进程不调用 `wait()`，那等 parent 终止后，这个僵尸进程可能仍然一直存在。此时这个僵尸进程同时是一个**孤儿进程(orphan)**，即没有 parent 进程的进程。这是不合理的。UNIX 的解决办法是，让所有孤儿进程都成为 `init`/`systemd` 的 child 进程，由 `init`/`systemd` 来 `wait()` 它们。
...
- 信号量(semaphores)：
    - 我们将在[进程同步这一章](./2-1-x.md#semaphores){target="_blank"}中重点讲解信号量；
- 共享内存(shared memory)：
...
<figure markdown>
<center>![Communications models. (a) Shared memory. (b) Message passing.](imgs/14.png)</center>
...
<figure markdown>
<center>![Diagram of process state.](imgs/20.png)</center>
Diagram of process state.
</figure>
...
!!! section "上下文切换"
    ...
    > 上下文切换的相关内容在 [lab1](https://zju-sec.github.io/os23fall-stu/lab1/#_11) 里有所涉及，需要实现一个简单的上下文切换。

<figure markdown>
<center>![(a) Diagram showing context switch from process to process. (b) Role of dispatcher.](imgs/16.png)</center>
...
- 周转时间(turnaround time)：
    - 从进程**开始建立**到进程完成的时间，即包括等待进入内存、在各种 queue 中的等待时间、在 CPU 中的运行时间、I/O 时间等，通过观察最大周转时间，能反映调度的效率和“公平性”；
- 等待时间(waiting time)：
...
???+ eg "🌰"
    ...
    === "解析"
        ...
        <center> ![](imgs/65.png){ width=80% } </center>
...
RR 调度是使用[分时技术](./0-x.md#time-sharing)的调度算法。由于时间片到期后，操作系统会强制停止进程，因此它是**抢占式**的。每一个进程最多连续执行一个时间片的长度，完成后被插入到 FIFO ready queue 的末尾，并取出 FIFO ready queue 的队首进行执行。

我们之前提到[分时](./0-x.md#time-sharing)的时候也说过，分时技术通过优化响应时间解决了用户交互问题，RR 调度虽然相比 SJF 有了更长的等待时间，但是有了更短的响应时间，而实际直接影响用户交互问题的应该是响应时间。
...
![Single-threaded and multithreaded processes.](imgs/18.png)
...
[^1]: [Where memory will be allocated to "Uninitialized Static variable" upon initialization?](https://stackoverflow.com/a/35799639 | StackOverflow){target="_blank"}
...
[^3]: [Linux CreateProcess?](https://stackoverflow.com/a/5883503 | StackOverflow){target="_blank"}
[^4]: [Differences between fork and exec?](https://stackoverflow.com/a/1653415 | StackOverflow){target="_blank"}